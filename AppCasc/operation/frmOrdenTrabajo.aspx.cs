    ...
    public partial class frmOrdenTrabajo : System.Web.UI.Page
    {
        ...
        private void refreshMaquilado(int IdEntradaInventario)
        {
            ...
            try
            {
                ...
                //if (diferencia != 0)
                //    udtRangeValidator(rv_pallet, 0, diferencia);
                //else
                //    txt_pallet.Enabled = false;
                diferencia = 0;

                //bultos
                //diferencia = oEI.Bultos - oEM.Bulto + Convert.ToInt32(txt_bulto.Text);
                //txt_bulto.Enabled = true;
                //if (diferencia != 0)
                //    udtRangeValidator(rv_bulto, 1, diferencia);
                //else
                //    txt_bulto.Enabled = false;
                //diferencia = 0;

                ////piezas
                //diferencia = oEI.Piezas - oEM.Pieza + Convert.ToInt32(txt_pieza.Text);
                //txt_pieza.Enabled = true;
                //if (diferencia != 0)
                //    udtRangeValidator(rv_pieza, 1, diferencia);
                //else
                //    txt_pieza.Enabled = false;
                //diferencia = 0;
                ...
                //IEnumerable<Entrada_inventario_detail> lstDifLote = lstEID.Distinct(new Entrada_inventario_detailLoteComparer());
                ...
                List<Entrada_maquila> lstMaq = EntradaCtrl.MaquilaSelByInventario(IdEntradaInventario);
                if (lstMaq.Count > 0)
                {
                    switch (lstMaq.First().Id_estatus)
                    {
                        //case Globals.EST_MAQ_PAR_CERRADA:
                        //case Globals.EST_REM_PARCIAL:
                        //    btn_cerrar_maquila.Text = "Re-Cerrar Orden";
                        //    break;
                        case Globals.EST_REM_TOTAL:
        ...
        private void fillPanelWork()
        {
            try
            {
                rep_WorkDay.DataSource = EntradaCtrl.InventarioGetByStatus(Globals.EST_INV_CON_APROBACION);
        ...
        protected void click_ord_cod(object sender, CommandEventArgs args)
        {
            try
            {
                ...
                Response.Redirect("frmOrdenTrabajo.aspx?_fk=" + IdEntrada.ToString() + "&_pk=" + IdEntrada_inventario.ToString());
            }
            catch (Exception e)
            {
                ((MstCasc)this.Master).setError = e.Message;
        ...
        protected void click_cerrar_maquila(object sender, CommandEventArgs args)
        {
            try
            {
                ...
                string msg = "Se cerro correctamente la orden de trabajo";
                ClientScript.RegisterStartupScript(this.GetType(), "alertSave", "<script type=\"text/javascript\">alert('" + msg + "');window.location.href='frmOrdenTrabajo.aspx?_fk=" + hf_id_entrada.Value + "&_pk=" + hf_id_entrada_inventario.Value + "';</script>");
            }
            catch (Exception e)
            {
                ((MstCasc)this.Master).setError = e.Message;
        ...
        protected void save_orden(object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                //window.location.href='frmOrdenTrabajo.aspx?_fk=" + hf_id_entrada.Value + "&_pk=" + hf_id_entrada_inventario.Value + "';
                btn_save.PostBackUrl = "frmOrdenTrabajo.aspx?_fk=" + hf_id_entrada.Value + "&_pk=" + hf_id_entrada_inventario.Value;
                fillEntradaInventario();
                fillPanelWork();
            }
            catch (Exception e)
        ...
        protected void change_date(object sender, EventArgs args)
        {
            ...
            try
            {
                ...
                Entrada_maquila o = EntradaCtrl.MaquilaSelBy(IdEntradaInventario, FechaTrabajo);
                if (o.Id >= 0)
                {
