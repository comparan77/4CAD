...
var BeanEntradaMaquilaDetail = function (id, idEntradaMaquila, danado, bultos, piezasxbulto, lote) {
    this.Id = id;
...
var MngOrdenTrabajo = function () {
    ...
    function init() {
        ...
        if (hf_id_entrada_inventario.val() != 0) {
            ...
            $('#btn_add_detalle').unbind('click').click(function () {
                ...
                var oEntMaqDet = new BeanEntradaMaquilaDetail(autoIncEntMaqDet++, 0, $('#thEstado').html() == 'Indemne' ? 0 : 1, bultos, piezasxbulto, lote);
        ...
        $(up_maquila).panelReady(function () {
            ...
            $('.diasTrabajados').children('li').each(function () {
                ...
                $(lnk).unbind('click').click(function () {
                    loadDetailByDate(hf_id_entrada_inventario.val(), $(this).html());
    ...
    function addDetail() {
        ...
        for (var item in lstEntMaqDet) {
            ...
            td += '<td align="center">' + lstEntMaqDet[item].Lote == 'null' ? '' : lstEntMaqDet[item].Lote + '</td>';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.Id > 0) {
                    ...
                    $('#hf_id_estatusMaquila').val(data.Id_estatus);
                    lstEntMaqDet = [];
                    $.each(data.LstEntMaqDet, function (i, obj) {
                        lstEntMaqDet.push(obj);
                    });

                    if ($('#hf_id_estatusMaquila').val() * 1 >= $('#ctl00_body_hf_EST_MAQ_PAR_CERRADA').val() * 1) {
                        $('#ctl00_body_btn_save').button('option', 'disabled', true);
                        $('#ctl00_body_btn_save').val('Maq. Cerrada.');
                    }
                }
                addDetail();
            },
