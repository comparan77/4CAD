...
{
    public partial class frmClienteCompradorLst : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs args)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    setCliente();
                    fillCatalog(CatalogCtrl.Cliente_compradorfillEvenInactive(Convert.ToInt32(hfFkey.Value)));
                }
                catch (Exception e)
                {
        ...
        protected void lnk_change_status_click(object sender, CommandEventArgs args)
        {
            try
            {
                ...
                o.Id = Id;
                CatalogCtrl.Cliente_compradorChangeStatus(o, status);
                fillCatalog(CatalogCtrl.Cliente_compradorfillEvenInactive(Convert.ToInt32(hfFkey.Value)));
            }
            catch (Exception e)
            {
